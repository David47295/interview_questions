...
function maximumContainers(scenarios) {
    ...
    for (let i = 0; i < scenarios.length; i++) {
        const variables = scenarios[i].split(' ');
        const budget = parseInt(variables[0]);
        const cost = parseInt(variables[1]);
        const threshold = parseInt(variables[2]);
        let num_containers = 0;

        num_containers += Math.floor(budget / cost);
        let remaining_containers = num_containers;
        while (remaining_containers >= threshold) {
            remaining_containers -= threshold - 1;
            num_containers++;
        }
...
function main() {
    for (let i = 0; i < test_cases.length; i++) {
        ...
        maximumContainers(test_cases[i]);
    }
}
